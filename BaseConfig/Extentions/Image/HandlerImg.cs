...
namespace BaseConfig.Extentions.Image
{
    ...
    {
        public async static Task<Dictionary<string, string>> UploadImgAsync(IConfiguration configuration, IFormFile imgData)
        {
            if (imgData == null || imgData.Length <= 0) return null;
            string accessKey = configuration.GetSection(ConstAppSettings.ENV_ACCESSKEY).Value;
            string secretKey = configuration.GetSection(ConstAppSettings.ENV_SERECT).Value;
            ...
            var request = new PutObjectRequest
            {
                ...
                Key = FolderSettingBase.IMG_USERS + $"{FolderSettingBase.DefaultNameImg}" + "_" + $"{imgData.FileName}",
        ...
        public static string GetLinkImg(IConfiguration configuration, string nameImg)
        {
